    ...
    subs.push(
      newSub && newSub !== true
        ? !oldSub || newSub[0] !== oldSub[0] || shouldRestart(newSub[1], oldSub[1])
          ? [newSub[0], newSub[1], (oldSub && oldSub[2](), newSub[0](dispatch, newSub[1]))]
          : oldSub
        : oldSub && oldSub[2](),
    );
  }
...
var makeDispatch = ({ subscriptions, dispatch = (x) => x, init, state, setState }) => {
  var subs = [];
  ...
  var update = (newState) => {
    if (state.current !== newState) {
      ...
      if (subscriptions) subs = patchSubs(subs, subscriptions(state.current), dispatch);
    }
  };
      ...
      typeof action === "function"
        ...
        : Array.isArray(action)
          ? typeof action[0] === "function"
            ...
            : action.slice(1).map((fx) => fx && fx !== true && (fx[0] || fx)(dispatch, fx[1]), update(action[0]))
          : update(action),
    ))(init),
    dispatch
  );
...
export default function (options) {
  ...
  const dispatch = useMemo(
    () => makeDispatch({ ...options, state, setState }), // can include {subscriptions}
    [],
  );
